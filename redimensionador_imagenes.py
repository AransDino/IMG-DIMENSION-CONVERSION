...
# Función para redimensionar las imágenes y agregar marca de agua
def redimensionar_imagen(ruta_imagen, ancho, alto, ruta_salida):
...
def procesar_imagenes():
    ...
    for archivo in os.listdir(carpeta):
        if archivo.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
            ...
            redimensionar_imagen(ruta_imagen, ancho, alto, ruta_salida)
            numerador += 1
...
style = ttk.Style()
style.configure("TButton", padding=6, relief="flat", background="#4F46E5", foreground="white")
...
button_procesar = ttk.Button(root, text="Seleccionar Carpeta y Procesar Imágenes", command=procesar_imagenes)
